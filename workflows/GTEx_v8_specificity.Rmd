...
> Code to generate this is originally from the paper [Bryois et al.](https://www.nature.com/articles/s41588-020-0610-9), available at: https://github.com/jbryois/scRNA_disease/blob/master/Code_Paper/Code_GTEx/get_GTEx_input.md. The original code has been modified below to: (1) Include all genes, as opposed to including only protein-coding genes; (2) not to add gene co-ordinates; (3) to make some column names more human-readable i.e. in the modified code below, what was "Lvl5" ("SMTSD" in GTEx) is now referred to as "Tissue", and what was "Tissue" ("SMTS" in GTEx) is referred to as "Organ".
...
```{r}
tissues <-
  read_delim("../misc_data/gtex_v8_tissues.tsv", delim = " ") %>% 
...
We then drop tissues with less than 100 samples, testis (gene expression outlier), non natural tissues (e.g. EBV-transformed lymphocytes and cultured fibroblasts) and then take the average expression for the different tissues that belong to the same organ.
...
saveRDS(exp_tissue, "../specificity_matrices/GTEx_v8.Rds")
```
