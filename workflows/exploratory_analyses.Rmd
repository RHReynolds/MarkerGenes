...
```{r run ewce all, echo = TRUE, eval = FALSE}
...
```{r show results of ewce, echo = FALSE, results = 'hide'}
...
ggplot(Zeisel_results %>% 
  ...
  facet_wrap(vars(GeneSet, Species, Study), scales = "free_x", ncol = 4) +
...
```{r show results of ewce for wang, echo = FALSE, results = 'hide'}
...
# Show only human and significant results
Wang_results_human %>% 
  dplyr::select(CellType, p, FDR, everything (), -annotLevel) %>% 
...
### TPM
```{r ewce plot wang TPM, echo = FALSE, fig.height = 10, out.width='100%', fig.pos='H', fig.cap = "***Figure:*** *Results of EWCE were filtered for only those cell types wherein the provided cell type markers (as detailed in the in the first label of each plot) had an FDR < 0.05. Labels found on top of each plot refer to (from top to bottom): 1) the cell type within the marker lists, and 2) the name of the study which provided the basis of the cell type data. Standard deviations from the mean is a measure of the distance between expression in the target list from the mean level of expression the bootstrapped sample. Within each plot, cell types are ordered by standard deviations from the mean.*"}

# Plot only significant results i.e. those where marker genes are more highly expressed in one cell type than you would expect by chance
ggplot(Wang_results_human %>% 
         dplyr::filter(str_detect(GeneSet, "TPM") & Species == "human") %>% 
         dplyr::select(CellType, p, FDR, everything (), -annotLevel) %>% 
         dplyr::mutate(GeneSet = str_replace(GeneSet, "TPM:", ""), Study = str_replace(Study, "_human", "")) %>% 
         filter(FDR < 0.05), aes(x =  MarkerGenes::reorder_within(x = CellType,
                                                                  by = sd_from_mean,
                                                                  within = GeneSet,
                                                                  fun = median,
  ...
  geom_col() +
  MarkerGenes::scale_x_reordered() +
  facet_wrap(vars(GeneSet, Study), scales = "free_x", ncol = 8) +
  labs(x = "Cell type", y = "Standard deviations from the mean", title = "") +
  theme_bw() +
  theme(axis.text.y = element_text(size = 6),
        axis.text.x = element_text(size = 6, angle = 45, vjust = 1, hjust = 1),
...
### UMI
```{r ewce plot wang UMI, echo = FALSE, fig.height = 16, out.width='100%', fig.pos='H', fig.cap = "***Figure:*** *Results of EWCE were filtered for only those cell types wherein the provided cell type markers (as detailed in the in the first label of each plot) had an FDR < 0.05. Labels found on top of each plot refer to (from top to bottom): 1) the cell type within the marker lists, and 2) the name of the study which provided the basis of the cell type data. Standard deviations from the mean is a measure of the distance between expression in the target list from the mean level of expression the bootstrapped sample. Within each plot, cell types are ordered by standard deviations from the mean.*"}

ggplot(Wang_results_human %>% 
         dplyr::filter(str_detect(GeneSet, "UMI") & Species == "human") %>% 
         dplyr::select(CellType, p, FDR, everything (), -annotLevel) %>% 
         dplyr::mutate(GeneSet = str_replace(GeneSet, "UMI:", ""), Study = str_replace(Study, "_human", "")) %>% 
         filter(FDR < 0.05), aes(x =  MarkerGenes::reorder_within(x = CellType,
                                                                  by = sd_from_mean,
                                                                  within = GeneSet,
                                                                  fun = median,
...
- UMI marker lists are more robust than TPM lists (although this probably reflects list size more than anything).
...
- Based on distributions of specificity indices (only in astrocytes), medians tend to be higher using the UMI human-derived marker lists, compared to the mouse-derived Zeisel list, indicating that using species-specific marker lists may be preferable.
