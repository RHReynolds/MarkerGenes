...
load("../specificity_matrices/Habib2017_DroNc_Mouse.rda")
load("../specificity_matrices/Skene2018.rda")

# Load list, gather and remove NAs.
Zeisel2015 <- read_delim("../flat_lists/Zeisel2015_TableS1_MarkerGenes.txt", delim = "\t") %>% 
  gather(key = "CellType", value = "Gene") %>% 
  na.omit()

Wang2018 <- read_excel(path = "../flat_lists/DER-19_Single_cell_markergenes_TPM.xlsx") %>% 
  dplyr::mutate(Technology = "TPM") %>% 
  bind_rows(read_excel(path = "../flat_lists/DER-21_Single_cell_markergenes_UMI.xlsx") %>% 
...
## Aim
- Develop a small brain-relevant marker gene resource for internal use.

# Curating the datasets
## Descriptions for metadata columns
```{r metadata columns, out.width='100%', fig.pos='H'}

readxl::read_excel(path = "../metadata/dataset_metadata.xlsx", sheet = "ColumnDescriptions") %>% 
...
## Dataset metadata
```{r metadata, out.width='100%', fig.pos='H'}

readxl::read_excel(path = "../metadata/dataset_metadata.xlsx", sheet = "Data") %>% 
  dplyr::arrange(desc(list_or_SI_matrix), resource_name) %>% 
  DT::datatable(rownames = FALSE,
                options = list(scrollX = TRUE),
                class = 'white-space: nowrap')
...
- The majority of specificity matrices were downloaded from Skene 2018, but also generated two matrices from:
    ...
    - Allen Brain Atlas 2019 (see: `../misc_scripts/20190501_AIBS2018_GeneratingSImatrix.R`). Used gene-level (exonic) read count values. While intronic values were available, reckoned that these would tag pre-mRNA.
...
```{r Zeisel2015 cell types, echo = T, class.source='fold-show'}
...
- Comparisons made:
    ...
    - AIBS2018: human and single nuclear. While this is middle temporal gyrus (i.e. temporal cortex), still cortical and considered the "gold standard".

```{r Zeisel2015 astrocyte numbers, echo = T, class.source='fold-show'}
...
```{r Zeisel2015 plot, echo = T, fig.height = 8, out.width='100%', fig.pos='H', fig.cap = "***Figure:*** *Displays specificity values of astrocyte marker genes derived from Zeisel 2015 across different cell types within various single-cell/nuclei studies. Within each study, cell types have been ordered by the median specificity.*"}

# Query across mouse and specificity matrices separately and combine dataframes.
Zeisel_SI <- 
  MarkerGenes::query_gene_ctd(genes = Zeisel2015 %>%
  ...
  bind_rows(MarkerGenes::query_gene_ctd(genes = Zeisel2015 %>%
                                          filter(CellType == "Astrocyte") %>% 
                                          .[["Gene"]], 
                                        ctd_DRONC_human, ctd_AIBS2018,
                                        celltypeLevel = 1,
                                        genelistSpecies = "mouse",
                                        ctdSpecies = "human") %>% 
...
MarkerGenes::summarise_specificity_plot(Zeisel_SI %>% 
...
```{r Zeisel2015 astro medians}

Zeisel_SI %>% 
  dplyr::group_by(Study, CellType) %>% 
  dplyr::summarise(Median = median(Specificity)) %>% 
  dplyr::filter(str_detect(CellType, "stro") | str_detect(CellType, "ASC"))
...
```{r Zeisel ewce all, echo = T, eval = FALSE, class.source = "fold-show"}
...
Zeisel_results <- 
  MarkerGenes::run_ewce_controlled(list_of_genes, reps = 10000, celltypeLevel = 1,
                                   genelistSpecies = "human", sctSpecies = "mouse",
                                   ctd_allKI, ctd_DRONC_mouse) %>%
  dplyr::mutate(Species = "mouse") %>%
  bind_rows(MarkerGenes::run_ewce_controlled(list_of_genes, reps = 10000, celltypeLevel = 1,
                                             genelistSpecies = "human", sctSpecies = "human",
                                             ctd_DRONC_human, ctd_AIBS2018) %>%
              dplyr::mutate(Species = "human"))

write_csv(Zeisel_results, path = "../results/20190515_EWCE_Zeisel2015_10000bootstrap.csv")
...
Zeisel_results <- 
  read_csv(file = "../results/20190515_EWCE_Zeisel2015_10000bootstrap.csv") %>% 
...
Zeisel_results %>% 
  ...
  dplyr::group_by(MarkerList, Study) %>% 
  dplyr::top_n(., n = 2, sd_from_mean) %>% 
  # Add those results where only 1 significant cell type found
  dplyr::bind_rows(Zeisel_results %>% 
              tidyr::unite(combined, c(GeneSet, Study), sep = ":", remove = FALSE) %>% 
              dplyr::filter(combined %in% c(cell_tally %>% 
                                               tidyr::unite(combined, c(GeneSet, Study), sep = ":", remove = FALSE) %>% 
                                               dplyr::filter(n < 2) %>% 
  ...
  dplyr::arrange(Study, MarkerList, desc(sd_from_mean)) %>% 
...
#### TPM
```{r Wang2018 TPM plot, fig.height = 8, out.width='100%', fig.pos='H', fig.cap = "***Figure:*** *Displays specificity values of astrocyte marker genes derived from Wang 2018 TPM across different cell types within various single-cell/nuclei studies. Within each study, cell types have been ordered by the median specificity.*"}

# Query across mouse and specificity matrices separately and combine dataframes.
Wang_SI <-  
  MarkerGenes::query_gene_ctd(genes = Wang2018 %>%
                              ...
                              ctdSpecies = "mouse") %>% 
  dplyr::mutate(Species = "mouse") %>% 
  dplyr::bind_rows(MarkerGenes::query_gene_ctd(genes = Wang2018 %>%
                                                 dplyr::filter(combined == "TPM:Adult-Astro") %>% 
                                                 .[["Gene"]], 
                                               ctd_DRONC_human, ctd_AIBS2018,
                                               celltypeLevel = 1,
                                               genelistSpecies = "human",
  ...
  dplyr::mutate(Technology = "TPM") %>% 
  dplyr::bind_rows(MarkerGenes::query_gene_ctd(genes = Wang2018 %>%
                                                 dplyr::filter(combined == "UMI:Astro") %>% 
                                                 .[["Gene"]], 
                                               ctd_allKI, ctd_DRONC_mouse,
                                               celltypeLevel = 1,
                                               genelistSpecies = "human",
...
# Summarise spread of SI across level 1 cell types from each study
MarkerGenes::summarise_specificity_plot(Wang_SI %>% dplyr::filter(Technology == "TPM"))

```

#### UMI
```{r Wang2018 UMI plot, fig.height = 8, out.width='100%', fig.pos='H', fig.cap = "***Figure:*** *Displays specificity values of astrocyte marker genes derived from Wang 2018 UMI across different cell types within various single-cell/nuclei studies. Within each study, cell types have been ordered by the median specificity.*"}

# Summarise spread of SI across level 1 cell types from each study
MarkerGenes::summarise_specificity_plot(Wang_SI %>% dplyr::filter(Technology == "UMI"))

```

#### Medians
```{r Wang2018 astro medians}

Wang_SI %>% 
  dplyr::group_by(Technology, Study, CellType) %>% 
  dplyr::summarise(Median = median(Specificity)) %>% 
  dplyr::filter(str_detect(CellType, "stro") | str_detect(CellType, "ASC"))
...
```{r run ewce all Wang, echo = T, eval = F, class.source = "fold-show"}
...
write_csv(Wang_results_human, path = "../results/20190523_EWCE_Wang2018_human_10000bootstrap.csv")

```

```{r Wang FDR, results = 'hide'}

# Add some form of multiple test correction
Wang_results_human <- read_csv(file = "../results/20190523_EWCE_Wang2018_human_10000bootstrap.csv") %>% 
  dplyr::mutate(FDR = p.adjust(p, method = "BH"),
                Study = str_replace(Study, "ctd_", ""))
...
Wang_results_human %>% 
  ...
  dplyr::top_n(., n = 2, sd_from_mean) %>% 
  # Add those results where only 1 significant cell type found
  dplyr::bind_rows(Wang_results_human %>% 
              dplyr::filter(FDR < 0.05 & str_detect(GeneSet, "TPM:")) %>% 
              tidyr::unite(combined, c(GeneSet, Study), sep = ":", remove = FALSE) %>% 
              dplyr::filter(combined %in% c(cell_tally %>% 
                                               tidyr::unite(combined, c(GeneSet, Study), sep = ":", remove = FALSE) %>% 
...
```{r non-significant marker lists}
...
- The majority of UMI markers are significantly enriched in more than one cell type, but typically show a preference for one cell type over the other. E.g. The Ex and In sub-categories are found signficantly enriched within both the glutamatergic and GABAergic classes from the AIBS2018 study, but preferentially enrich in the class you would expect (i.e. Ex in glutamatergic and In in GABAergic).
...
## EWCE: UMI Wang marker lists and AIBS2018 level 2 cell types
...
- Furthermore, focussed on the AIBS2018 study as there are 75 different level 2 cell types, as opposed to only 15 in the DRONC study, so there is a real opportunity to put the lists to the test in distinguishing between level 2 cell types.

```{r run ewce UMI Wang level 2, echo = T, eval = F, class.source = "fold-show"}
...
# Running EWCE only in level 2 cell types and in AIBS2018
Wang_results_UMI <- run_ewce_controlled(list_of_genes, reps = 10000, celltypeLevel = 2,
                               genelistSpecies = "human", sctSpecies = "human",
                               ctd_AIBS2018)
write_csv(Wang_results_UMI, path = "../results/20190523_EWCE_Wang2018_UMI_AIBS2018_10000bootstrap.csv")

```

```{r UMI results, echo = T}

# Apply multiple test correction
Wang_results_UMI <- read_csv(file = "../results/20190523_EWCE_Wang2018_UMI_AIBS2018_10000bootstrap.csv") %>% 
  dplyr::mutate(FDR = p.adjust(p, method = "BH"))
...
- Species-matched lists appear to perform better as based on the followed observations:
    ...
    2. A comparison of the median fold difference of the mouse-derived Zeisel and human-derived UMI marker in the AIBS2018  study (using only level 1 cell types) found the median fold difference to be higher when using the species-matched list (UMI, median = 3.77; Zeisel, median = 2.92).
